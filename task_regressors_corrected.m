...
function [] = task_regressors_corrected(s)
%clear all
...
%         end
        ...
        durations{1} = C_duration.Duration(markersNP)+1; % add 1 sec until speech starts
        durations{2} = C_duration.Duration(markersSC)+1;
        durations{3} = C_duration.Duration(markersSE)+1;
        durations{4} = C_duration.Duration(markersFIL)+1;
%         
%         pmod = struct('name',{''},'param',{},'poly',{});
%         pmod(1).name{1}  = 'NPonset';
